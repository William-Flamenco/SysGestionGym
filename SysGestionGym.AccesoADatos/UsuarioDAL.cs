    ...
    public class UsuarioDAL
    {
        ...
        public static async Task<int> CrearAsync(Usuario pUsuario)
        {
            ...
            using (var bdContexto = new BDContext())
            {
                ...
                if (!existeLogin)
                {
                    pUsuario.FechaNacimiento = DateOnly.FromDateTime(DateTime.Now);
        ...
        public static async Task<Usuario> ObtenerPorIdAsync(int idUsuario)
        {
            ...
            using (var bdContexto = new BDContext())
            {
                usuario = await bdContexto.Usuario.FirstOrDefaultAsync(s => s.IdUsuario == idUsuario);
            }
            return usuario;
        }
        ...
        internal static IQueryable<Usuario> QuerySelect(IQueryable<Usuario> pQuery, Usuario pUsuario)
        {
            ...
            {
                DateOnly fechaInicial = pUsuario.FechaNacimiento;
        ...
        public static async Task<int> CambiarPasswordAsync(Usuario pUsuario, string pPasswordAnt)
        {
            ...
            {
                var usuario = await bdContexto.Usuario.FirstOrDefaultAsync(s => s.IdUsuario == pUsuario.IdUsuario);
                if (usuario != null && usuarioPassAnt.Password == usuario.Password)
                {
                    EncriptarMD5(pUsuario);
